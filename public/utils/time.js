const { num_pad_left } = require('./format');
 ...
 * @memberOf mUtils.time
 */
function today(zeroTime = true) {
  return getTime({ zeroTime });
}
 ...
 * @memberOf mUtils.time
 */
function tomorrow(zeroTime = true) {
  return getTime({ zeroTime, offset: _aDay });
}
 ...
 * @memberOf mUtils.time
 */
function yesterday(zeroTime = true) {
  return getTime({ zeroTime, offset: -_aDay });
}
...
function date_range(rangeNums, from = today()) {
  let days = parseInt(rangeNums) * _aDay,
